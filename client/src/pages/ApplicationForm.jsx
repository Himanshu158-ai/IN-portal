...
function ApplicationForm() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const popup = {
        id: Date.now(),
        text: govtMessages[Math.floor(Math.random() * govtMessages.length)],
        top: Math.random() * 80,
        left: Math.random() * 70,
      };
  ...
  const fakeSubmit = () => {
    if (!checked) {
      setMessage("Please confirm patriotism checkbox before proceeding.");
      return;
    }
    ...
    const interval = setInterval(() => {
      ...
      if (value >= 100) {
        ...
        setTimeout(() => {
          setLoading(false);
          setMessage("Wrong button detected. Find the correct submit button!!!");
        }, 1500);
        return;
      }
  ...
  const realSubmit = () => {
    setMessage("Citizen verification submitted successfully");
  };
      ...
      ">
        ...
        ">
                ...
                {field.includes("Zone") ||
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <option>National Sector Alpha</option>
                      <option>Central Verification Unit</option>
                      <option>Regional Identity Cluster</option>
                      <option value="DONE_SUBMIT">done</option>
                    </select>
                  </div>
                ) : (
                  <input
          ...
          <div className="mt-10 sm:mt-12">
            <h3 className="text-xl sm:text-2xl font-black text-red-700 mb-4">
              SECURITY CAPTCHA
            </h3>
            <div className="
              flex
          ...
          {/* Patriotism checkbox */}
          <div className="mt-12 sm:mt-16 flex items-center gap-4 relative">
            <motion.input
              animate={{
                x: [0, 80, -60, 40, 0],
                y: [0, -10, 10, -5, 0],
              ...
              checked={checked}
              readOnly
              className="w-6 h-6 pointer-events-none shrink-0"
            />
          ...
          {loading && (
            <div className="mt-12 sm:mt-16">
              <div className="flex justify-between mb-2 font-bold text-sm sm:text-base">
                <span>Uploading citizen identity...</span>
                <span>{progress}%</span>
              </div>
              <div className="
                w-full
            ...
            <motion.button
              onMouseEnter={() => {
                if (window.innerWidth > 768) {
                  setButtonX(Math.random() * 300 - 150);
                }
              }}
              animate={{ x: buttonX }}
              whileTap={{ scale: 0.9 }}
